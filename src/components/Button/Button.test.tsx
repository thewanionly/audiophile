...
describe('Button', () => {
  describe('As button element', () => {
    ...
    `('sets background-color to $bgColor when `color` prop is $color', ({ color, bgColor }) => {
    ...
    `(
      ...
      ({ variant, propertyToColor }) => {
        ...
        expect(button).toHaveStyle(`${propertyToColor}: ${theme.colors.primary}`)
      }
    )
  ...
  describe('As link element', () => {
    ...
    it('contains a string equivalent to the `download` prop', () => {
      ...
      render(<Button asLink label={buttonLabel} href={href} download={download} />)
